...
st.set_page_config(
    layout='wide', page_title='ATS Resume Canvas Studio', page_icon='📄'
)
...
def generate_ats_pdf(data, config, t):
  ...
  font_regular, font_bold = font_map.get(
      font_family, ('Helvetica', 'Helvetica-Bold')
  )
  ...
  if data.get('projects'):
    ...
    for proj in data['projects']:
      header_proj = (
          f"<b>{sanitize(proj['title'])}</b> <i>({sanitize(proj['tech'])})</i>"
      )
...
with st.sidebar:
  ...
  selected_lang = st.selectbox(
      'Seleccionar Idioma del Documento', options=list(I18N.keys()), index=0
  )
  ...
  font_choice = st.selectbox(
      'Tipografía ATS', options=['Helvetica', 'Times-Roman', 'Courier'], index=0
  )
  base_font_size = st.slider(
      'Tamaño Fuente', min_value=8.0, max_value=12.0, value=9.5, step=0.5
  )
  margin_size = st.slider(
      'Márgenes (pt)', min_value=20, max_value=60, value=35, step=5
  )

  st.markdown('---')
  st.header('💾 Backup / Restaurar')
  uploaded_json = st.file_uploader('Cargar JSON de CV', type=['json'])
...
st.markdown(
    "<span class='ats-badge'>Multilingual ATS Compliance</span>",
    unsafe_allow_html=True,
)
...
with col_canvas:
  ...
  with st.expander(t['sec_contact'], expanded=True):
    ...
    linkedin = st.text_input(
        'LinkedIn / Portfolio URL', 'linkedin.com/in/ana-biomedica'
    )
  ...
  with st.expander(t['sec_profile'], expanded=True):
    summary = st.text_area(
        ...
        height=80,
        value=(
            'Ingeniera biomédica enfocada en el desarrollo de software médico,'
            ' análisis de señales biológicas y cumplimiento normativo'
            ' (NOM-004-SSA3-2012). Experiencia en Python, MATLAB y herramientas'
            ' clínicas interactivas.'
        ),
    )
  ...
  with st.expander(t['sec_exp'], expanded=True):
    ...
    for i in range(st.session_state.exp_count):
      ...
      role = ec1.text_input(
          f'Título del Cargo {i+1}',
          'Desarrolladora de Software Médico',
          key=f'role_{i}',
      )
      company = ec2.text_input(
          f'Empresa {i+1}', 'Hospital / Red Médica', key=f'comp_{i}'
      )
      period = st.text_input(
          f'Periodo {i+1}', '2024 - Presente', key=f'per_{i}'
      )
      resp = st.text_area(
          f'Responsabilidades {i+1}',
          'Desarrollo de sistema web en Streamlit para pre-auditoría'
          ' clínica.\nImplementación de algoritmos en Python para filtrado de'
          ' señales fisiológicas.\nOptimizó la revisión de expedientes clínicos'
          ' en un 35%.',
          key=f'resp_{i}',
          height=80,
      )
  ...
  with st.expander(t['sec_proj'], expanded=False):
    ...
    for j in range(st.session_state.proj_count):
      ...
      p_title = pc1.text_input(
          f'Nombre {j+1}',
          'Sistema de Hemocitaféresis Magnética (HMS)',
          key=f'ptr_{j}',
      )
      p_tech = pc2.text_input(
          f'Tecnologías {j+1}',
          'Python, Nanoflores de Magnetita',
          key=f'ptch_{j}',
      )
      p_details = st.text_area(
          f'Descripción {j+1}',
          'Diseño de prototipo conceptual para extracción de endotoxinas en'
          ' sangre.\nSimulación de mecánica de fluidos y cálculos de tasas de'
          ' captura de magnetita.',
          key=f'pdet_{j}',
          height=70,
      )
      projects.append(
          {'title': p_title, 'tech': p_tech, 'details': p_details}
      )
      st.divider()
  ...
  with st.expander(t['sec_skills'], expanded=True):
    skills = st.text_area(
        'Skills',
        value=(
            'Python, MATLAB, Streamlit, NOM-004-SSA3-2012, Procesamiento de'
            ' Señales (ECG), Git, PyVis, Regex, Análisis de Datos Clínicos.'
        ),
    )
  ...
  with st.expander(t['sec_lang'], expanded=False):
    languages = st.text_area(
        ...
        value=(
            'Español (Nativo), Inglés (C1 Avanzado), Francés (B1 Intermedio),'
            ' Certificación Introducción a IA - Iberoamerican Technology.'
        ),
    )
  ...
  with st.expander('🤖 Feedback de Vacante con IA (Gemini)', expanded=False):
    ...
    st.info(
        ...
        'El análisis de compatibilidad es generado de forma automatizada'
        ' mediante IA. Los modelos automatizados pueden cometer errores o tener'
        ' imprecisiones. Utiliza los resultados como una guía de referencia y'
        ' no como un dictamen definitivo.'
    )
    ...
    uploaded_cv = st.file_uploader(
        'Sube tu archivo de CV', type=['pdf', 'txt'], key='ai_cv_uploader'
    )

    st.divider()
    job_offer = st.text_area(
        'Pega aquí la descripción completa de la vacante:', height=120
    )
    ...
    if btn_analyze:
      if not api_key:
        st.error(
            '⚠️ Falta configurar la `GEMINI_API_KEY` en'
        ...
        with st.spinner(
            '🤖 Procesando CV y analizando métricas con Gemini...'
        ):
          try:
            cv_text_to_analyze = ''
            ...
            if not cv_text_to_analyze.strip():
              ...
              # Sanitización contra Markdown accidental
              ...
              with col_m1:
                ...
                if score >= 80:
                  ...
                  st.warning(
                      'Buena coincidencia, requiere afinar detalles.'
                  )
                else:
              ...
              st.divider()
              st.caption(
                  '⚠️ *Aviso de responsabilidad:* Las métricas y sugerencias'
                  ' presentadas son generadas por Inteligencia Artificial. Revisa'
                  ' manualmente las recomendaciones antes de enviar tu'
                  ' candidatura.'
              )
...
with col_preview:
  ...
  pdf_display = (
      ...
      ' height="550" type="application/pdf" style="border-radius:8px;'
      ' border:1px solid #30363D; background-color: white;"></iframe>'
  )
  ...
  with c_btn1:
    st.download_button(
        ...
        data=pdf_bytes,
        file_name=(
            f"CV_{full_name.replace(' ', '_')}_{selected_lang[:2]}.pdf"
        ),
